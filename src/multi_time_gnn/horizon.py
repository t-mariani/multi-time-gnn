...
from multi_time_gnn.dataset import Normalizer, TimeSeriesDataset
from multi_time_gnn.visualization import plot_prediction_horizons


log = get_logger()

def horizon_computing(model, test, config, normalizer:Normalizer, list_horizon=None):
    ...
    with torch.no_grad():
        for k, (x, y) in tqdm(enumerate(dataloader_test), total=min(len(dataloader_test), config.nb_test) - 1):
            for j in range(max_horizon):
                ...
                y_denorm_pred = normalizer.denormalize(y_pred.squeeze().to("cpu").T).T
                y_denorm_true = normalizer.denormalize(y_true.squeeze().to("cpu").T).T
    ...
    signal_horizons_denorm = normalizer.denormalize(signal_horizons)
    test_denorm = normalizer.denormalize(test)
